...
void draw(){
  switch(screen){
    ...
    case "menu":
      ...
      textSize(12);
      text("experimental", width/5*3.91, 135);
...
void botcontrol(){
  ...
  float bestdistance = 0;
	PVector bestmovement = new PVector(0, 0);
  ...
  if(healdist < 65 && healer.pos.x < botpos.x && distance > 17 && bhp < 120){
    ...
    for(int i = 0; i < bullets.size() - 1; i ++){
      ...
      buls.add(new fakebullet(b.pos.x + b.vel.x * 3, b.pos.y + b.vel.y * 3));
    }
    for(int x = -2; x < 3; x += 2){
      for(int y = -2; y < 3; y += 2){
    ...
    botpos.x = constrain(botpos.x + bestmovement.x, width/3*2 - boxsize.x / 2 + bwid + 6, width/3*2 + boxsize.x / 2 - bwid - 5);
    botpos.y = constrain(botpos.y + bestmovement.y, 400 - boxsize.y/2 + bwid + 6, 400 + boxsize.y/2 - bwid - 5);
  }
}
